...
function four(a, b, c, d) {
  ...
  return pts.every(Boolean) ? ringCorners(pts) : null;
}

export function ringCorners(corners) {
  if (!corners || corners.length < 4) return corners || null;
  const pts = corners.slice(0, 4);
  const cx = pts.reduce((s, p) => s + p.x, 0) / pts.length;
  const cy = pts.reduce((s, p) => s + p.y, 0) / pts.length;
  return pts
    .map((p, i) => ({ p, a: Math.atan2(p.y - cy, p.x - cx), i }))
    .sort((a, b) => a.a - b.a || a.i - b.i)
    .map((x) => x.p);
}
...
export function smoothQrCorners({ prev, next, alpha = 0.82, deadzone = 7 } = {}) {
  if (!next || next.length < 4) return null;
  if (!prev || prev.length !== next.length) return next;
...
export function detectArea(corners) {
...
export function quadCentroid(corners) {
  ...
  return {
    x: pts.reduce((s, p) => s + Number(p.x), 0) / pts.length,
    y: pts.reduce((s, p) => s + Number(p.y), 0) / pts.length,
  };
}

export function pickPrimaryDetect(hits, { prefer } = {}) {
  const list = Array.isArray(hits) ? hits : [];
  const found = [];
  for (const hit of list) {
    const code = String((hit && (hit.rawValue || hit.data)) || '').trim();
    if (!code) continue;
    const corners = cornersFromDetect(hit);
    if (!corners) continue;
    found.push({ code, corners, area: detectArea(corners) });
  }
  if (!found.length) return null;
  const want = String(prefer || '').trim().toLowerCase();
  if (want) {
    const kept = found.find((h) => h.code.toLowerCase() === want);
    if (kept) return { code: kept.code, corners: kept.corners };
  }
  found.sort((a, b) => b.area - a.area || a.code.localeCompare(b.code));
  return { code: found[0].code, corners: found[0].corners };
}
